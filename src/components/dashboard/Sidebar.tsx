...
import { Briefcase, PlusCircle, Building2, User, LogOut, LayoutDashboard} from "lucide-react"; // icon lucide-react


const menu = [
  { name: "Dashboard", to: "/dashboard/mainDashboard", icon: LayoutDashboard },
  { name: "Jobs", to: "/dashboard/jobs", icon: Briefcase },
  { name: "Create Job", to: "/dashboard/create-job", icon: PlusCircle },
  { name: "Company Settings", to: "/dashboard/company", icon: Building2 },
];
      ...
      <ul className="space-y-2 flex-1">
        {menu.map(({ name, to, icon: Icon }) => (
          <li key={to}>
            ...
            >
              <Icon className="w-5 h-5" /> {/* render komponen */}
              <span>{name}</span>
            </NavLink>
          </li>
        ))}
